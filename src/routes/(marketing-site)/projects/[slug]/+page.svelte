<script lang="ts">
  ...
  $: project = data.isProject ? (getProjectBySlug($page.params.slug) || projectsMap[$page.params.slug]) : null;
...
<svelte:head>
  ...
  {:else if data.isFilter}
    <title>{data.filterName} Projects - Techyor</title>
    <meta name="description" content="Explore our portfolio of projects related to {data.filterName}." />
  {:else}
  ...
  <main class="overflow-hidden">
    ...
    <div class="container mx-auto mt-10 px-container md:mt-14 lg:mt-20">
      <div class="flex items-center gap-3 mb-6">
        <a href="/projects" class="text-foreground-secondary hover:text-foreground transition-colors">Projects</a>
        <span class="text-foreground-secondary">/</span>
        <h1 class="text-5xl md:text-6xl lg:text-7xl capitalize">{data.filterName}</h1>
      </div>
      <p class="text-foreground-secondary max-w-2xl">
        Explore our {filteredProjects.length} {filteredProjects.length === 1 ? 'project' : 'projects'} related to {data.filterName}
      </p>
    </div>
