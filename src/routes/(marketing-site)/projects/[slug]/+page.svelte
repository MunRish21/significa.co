  ...
  <main>
    ...
    <div class="container mx-auto px-container">
      ...
      <div class="group rich-text my-20 @container">
        ...
        <!-- Gisteo Content -->
        {#if project.slug === 'gisteo'}
          <h2 class="mx-auto max-w-2xl"><b>The challenge.</b></h2>
          <p class="mx-auto max-w-2xl">
            Video content has become essential for SaaS companies, marketing teams, and startups. But traditional video production is expensive, slow, and requires specialized talent: directors, animators, voiceover artists, editors. A single explainer video might take weeks to produce and cost $5,000-$20,000. This creates a bottleneck: companies with limited budgets resort to generic stock footage or no video at all, missing opportunities to communicate product value, explain complex concepts, and connect emotionally with audiences.
          </p>

          <p class="mx-auto max-w-2xl">
            Gisteo was founded to solve this problem by putting high-quality video production into the hands of product teams, founders, and marketers. The challenge was building a platform sophisticated enough to handle the technical complexity of video generation—AI avatars that speak naturally, animated graphics that sync to narration, dynamic templating that scales across use cases—while remaining intuitive enough for non-technical users to create professional results in minutes, not months.
          </p>

          <h2 class="mx-auto max-w-2xl"><b>Building an AI-powered video production platform.</b></h2>
          <p class="mx-auto max-w-2xl">
            We engineered a comprehensive SaaS platform that simplifies video creation without sacrificing quality. The architecture combines three core components: an intuitive web editor where users compose video narratives, an AI engine that intelligently generates video content, and a high-performance rendering pipeline that produces broadcast-quality output at scale.
          </p>

          <p class="mx-auto max-w-2xl">
            The editor itself is the product's central nervous system. Users start by selecting a template tailored to their use case—explainer video, SaaS demo, founder story, product launch announcement—or building from scratch using a drag-and-drop scene composer. Each template comes pre-configured with layout guidelines, animation patterns, and pacing that reflect best practices from professional video production, so even first-time users produce coherent, well-paced narratives.
          </p>

          <h2 class="mx-auto max-w-2xl"><b>AI-powered avatar generation and intelligent automation.</b></h2>
          <p class="mx-auto max-w-2xl">
            The platform's standout feature is its AI avatar system. Users can input a script, select from a diverse roster of realistic AI avatars with distinct personalities and vocal qualities, and the system automatically generates a video of the avatar speaking the script with natural gestures, facial expressions, and lip-sync. The AI understands emotion, pacing, and intent—a founder explaining a vision gets different timing and expression than a customer support representative walking through a feature.
          </p>

          <p class="mx-auto max-w-2xl">
            Beyond avatars, the platform offers intelligent automation across the entire production pipeline. Input a product concept and the system suggests visuals, animations, and transitions. Describe a complex workflow and Gisteo auto-generates annotated screen recordings that sync to narration. Voice generation with natural prosody and accentuation eliminates the need to hire voiceover talent. Background music, sound effects, and ambient audio are intelligently selected and mixed to enhance the narrative without overwhelming it.
          </p>

          <h2 class="mx-auto max-w-2xl"><b>Flexible templating for every video type.</b></h2>
          <p class="mx-auto max-w-2xl">
            The platform ships with eight specialized templates, each tuned for a distinct use case: explainer videos (concept-focused, educational), SaaS demos (feature-focused, technical), AI avatar narratives (personal, conversational), animated storytelling (brand-focused, emotional), product launch announcements (announcement-focused, excitement-driven), startup pitch videos (pitch-focused, persuasive), customer testimonials (social proof, authentic), and founder stories (leadership-focused, visionary).
          </p>

          <p class="mx-auto max-w-2xl">
            Each template comes pre-configured with segment lengths, visual styles, animation libraries, and audio cues tuned to maximize engagement for its specific context. Users can customize within templates or remix across them, giving creative teams the structure to move fast without sacrificing uniqueness.
          </p>

          <h2 class="mx-auto max-w-2xl"><b>Scalable rendering and intelligent asset management.</b></h2>
          <p class="mx-auto max-w-2xl">
            Behind the scenes, Gisteo manages a sophisticated rendering architecture that converts editor compositions into broadcast-quality video. The system optimizes for multiple output formats—16:9 for YouTube and web, 9:16 for TikTok and Instagram Reels, 1:1 for LinkedIn, 4:3 for older platforms—allowing a single composition to generate multiple cuts for different channels automatically.
          </p>

          <p class="mx-auto max-w-2xl">
            Asset management is intelligent: the platform caches and reuses rendered components, intelligent scheduling spreads rendering load to minimize queue times, and progressive quality encoding ensures users can preview results quickly while still accessing full-resolution output for production use. A typical video renders in minutes, not hours.
          </p>

          <h2 class="mx-auto max-w-2xl"><b>Workflow automation and enterprise integration.</b></h2>
          <p class="mx-auto max-w-2xl">
            For teams managing video at scale, Gisteo offers workflow automation that connects to existing tools. Bulk import scripts, select avatars and styles, and automatically generate dozens of video variations in parallel. API integrations allow teams to programmatically generate videos on-demand—a marketplace could auto-generate product videos for every new seller, a SaaS app could create onboarding videos dynamically for each customer, a content studio could queue productions based on editorial calendar.
          </p>

          <p class="mx-auto max-w-2xl">
            Collaboration tools built into the platform allow creative teams to comment, suggest revisions, and iterate together without leaving the editor. Version history and branching let teams explore multiple directions in parallel and merge the best ideas.
          </p>

          <h2 class="mx-auto max-w-2xl"><b>User experience designed for creators, not video engineers.</b></h2>
          <p class="mx-auto max-w-2xl">
            The user experience reflects a core principle: video production complexity should be invisible. Users interact with familiar creative tools—a timeline view for pacing, a properties panel for customization, a preview monitor for real-time feedback. Technical concepts like codec selection, bitrate management, and color grading are abstracted behind smart defaults that users can tweak if needed but rarely must.
          </p>

          <p class="mx-auto max-w-2xl">
            Onboarding is guided: templates include example projects with pre-populated content that users learn from by editing. In-editor tips surface best practices for each template type. A built-in inspiration library showcases videos created by other users, demonstrating what's possible and providing tactical inspiration for composition, pacing, and style.
          </p>

          <h2 class="mx-auto max-w-2xl"><b>Enterprise-grade reliability and performance.</b></h2>
          <p class="mx-auto max-w-2xl">
            Gisteo handles variable workloads: individual creators making occasional videos and enterprise teams generating hundreds daily. The architecture scales horizontally across rendering nodes, load balances requests across edge locations, and prioritizes production jobs with intelligent queuing. SLA guarantees ensure critical rendering jobs complete within guaranteed timeframes.
          </p>

          <p class="mx-auto max-w-2xl">
            Security is built in: role-based access controls allow teams to manage who can create, edit, publish, and export. Brand asset libraries ensure on-brand colors, logos, and fonts are always accessible and up-to-date. Audit logs track every change, and compliance integrations support GDPR, SOC 2, and other regulatory requirements important to enterprise customers.
          </p>

          <h2 class="mx-auto max-w-2xl"><b>Building a creator community.</b></h2>
          <p class="mx-auto max-w-2xl">
            Beyond the platform itself, Gisteo has built a community of creators sharing templates, techniques, and inspiration. The inspiration marketplace lets users browse videos created by others, remix successful compositions, and discover new approaches. This network effect creates value: each new template shared by one user becomes available to thousands, and the platform becomes smarter as it learns from aggregate usage patterns.
          </p>

          <p class="mx-auto max-w-2xl">
            Educational content—tutorials, best-practice guides, case studies—helps users accelerate from their first video to producing video at scale. Live workshops and office hours connect creators with the core team, creating feedback loops that drive continuous product improvement.
          </p>

          <h2 class="mx-auto max-w-2xl"><b>Results and impact.</b></h2>
          <p class="mx-auto max-w-2xl">
            Since launch, Gisteo has empowered thousands of teams to produce video content at scale. Startups use it to create compelling pitch videos and product demos that attract investors and customers. SaaS companies generate video-driven onboarding flows that improve feature adoption. Marketing teams produce dozens of video variations for A/B testing across channels. Agencies use Gisteo as a production tool that lets them deliver video projects to clients faster and at lower cost, improving margins and enabling them to serve more customers.
          </p>

          <p class="mx-auto max-w-2xl">
            The platform has become a productivity multiplier for content teams. What once required a week-long production sprint and $10,000 in talent costs now takes hours and costs a fraction of the price. Teams that historically couldn't afford video now weave it throughout their marketing funnel. The combination of intuitive editing, intelligent automation, AI-powered generation, and enterprise-grade reliability has democratized high-quality video production for the modern web.
          </p>

          <p class="mx-auto max-w-2xl">
            The long-term vision for Gisteo is clear: become the canonical production tool for video content in the AI era, used by individual creators, growing teams, and enterprises alike—the same way Figma became the canonical tool for design, and Zapier became the canonical tool for automation. As AI capabilities evolve, Gisteo's architecture is positioned to evolve with them: new avatar models with better naturalness, multimodal AI that generates visuals from text, audio processing that learns speaker intent and adapts automatically. The platform is built to improve continuously as the underlying technology improves.
          </p>
        {/if}
