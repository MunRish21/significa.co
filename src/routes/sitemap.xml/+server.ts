...
export const GET = async () => {
  // Published blog posts — fetched at request time so newly published posts
  // appear in the sitemap without a redeploy. Cache headers below bound the
  // freshness window.
  let blogPages: { path: string; priority: number; changefreq: string; lastmod: string }[] = [];
  try {
